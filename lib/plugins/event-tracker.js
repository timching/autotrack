...
class EventTracker {
  ...
  handleEvents(event, element) {
    this.queue.pushTask(({time}) => {
      ...
      if (Array.isArray(window[this.opts.dataLayer])) {
          window[this.opts.dataLayer].push({
            event: attributeFields.eventAction || '',
            label: attributeFields.eventLabel || '',
            category: attributeFields.eventCategory || '',
          });
      }
    });
  }
